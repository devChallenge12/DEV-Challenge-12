...
- (void)detectingFaceImage:(UIImage *)img completion:(void (^)(CGRect, NSString *))result {
   
    dispatch_async(_faceTrackingQueue, ^{
        @autoreleasepool{
            
            CIImage* image = [[CIImage alloc] initWithCGImage:img.CGImage];
            //face tracking
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.6 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
           [self detectingFaceImage:img completion:result];
        });
